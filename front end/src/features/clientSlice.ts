...
export const fetchClientsCount = createAsyncThunk<ClientsCount, void>(
  "clients/fetchClientsCount",
  async () => {
...
export const fetchTotallAccountBalance = createAsyncThunk<TotallBalances, void>(
  "clients/fetchTotallAccountBalances",
  async () => {
...
>("client/deposit", async ({ accountNumber, amount }, { rejectWithValue }) => {
  try {
...
export const withdrewAmount = createAsyncThunk<
  ...
  DepositParams, // Input type
  { rejectValue: string } // Error type
>("client/withdrew", async ({ accountNumber, amount }, { rejectWithValue }) => {
  try {
...
>(
  "client/Transfer",
  async (
    { message, fromAccountNumber, toAccountNumber, amount },
    { rejectWithValue },
  ) => {
    try {
...
const clientSliceReducer = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        TransferFund.fulfilled,
        (state, action: PayloadAction<TransferFundResponse>) => {
          state.status = "succeeded";
          state.loading = false;
          state.error = null;
...
export const { setAccountNumber, resetClientState } =
  clientSliceReducer.actions;
